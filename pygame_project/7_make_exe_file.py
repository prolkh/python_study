...
## pyinstaller -w --add-data '.\python_study\pygame_project\images\*.png;images'  .\python_study\pygame_project\7_make_exe_file.py
## 
## -w : 윈도우모드로 exe 생성
## --add-data 'file;dest' : 이미지, 폰트 같은 자원 추가에 사용
## -F : 하나의 exe 파일로 만들때 사용
##
## 결과 파일 생성 위치: dist
...
# 상대경로를 사용하는 경우 exe 파일을 만드려면 아래의 함수를 이용하여 이미지 파일 경로를 설정해주어야한다.
...
# Font 정의
game_font = pygame.font.SysFont('malgungothic', 40)      # 폰트종류 및 폰트크기
total_time = 10
start_ticks = pygame.time.get_ticks() # 시작 시간 정의

# 게임 종료 메시지 / TimeOut(시간초과 실패), Complet(성공) , Game Over(캐릭터 공에 맞음, 실패)
game_result = "Game Over!ㅋ"
...
while running:
    ...
    # 천장에 닿은 무기 없애기
    weapons = [ [w[0], w[1]] for w in weapons if w[1] > 0]      # 천장에 닿지 않은 무기만 위치정보 다시 저장
    ...
    if len(balls) == 0:
        game_result = "Misson Complete 올ㅋ"
        running = False

...
def merge_image() :
    try:
        ...
        image_sizes = [] # [(width1, height1), (width1, height1), (width1, height1)]
        if img_width > -1:
            # width 값 변경
        ...
        if img_space > 0: # 이미지 간격 옵션 적용
            tot_height += (img_space * (len(images) - 1))
        ...
        # 포맷 옵션 처리
        file_name = "image{}.{}".format(time.strftime("_%Y%m%d_%H%M%S"), img_format) # 경로 내 파일 이름 중복방지 시간으로 생성
        print(file_name)
